...
getLinkCommunities <- function(x, plot = TRUE, hcmethod = "average", edglim = 10^4, directed = FALSE, dirweight = 0.5, removetrivial = TRUE) 
	...
	if(plot){
		...
		if(len < 1500){ # Will be slow to plot dendrograms for large networks.
			if(len < 500){
				all <- plot(all, type="summary")
			}else{
				all <- plot(all, type="summary", right = FALSE) # Slow to reverse order of large dendrograms.
				}
		}else if(len <= edglim){
