...
\noindent Three networks are provided as part of the package so that users can see how the functions work before using their own data:
...
Several other options are available and are described in {\tt ?getLinkCommunities} and also in {\tt ?igraph.plotting} in the {\tt igraph} R package.
