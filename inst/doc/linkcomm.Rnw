%\VignetteIndexEntry{linkcomm}
%\VignetteDepends{grid, igraph, RColorBrewer}
%\VignetteKeywords{linkcomm}
...
The R package {\tt linkcomm} implements the algorithm proposed by \cite{ahnetal2010}. Similarities between links that share a node are assigned using the Jaccard coefficient and these similarities are then used to hierarchically cluster the links. The resulting dendrogram is cut at a point that maximises the density of links within the clusters after normalizing against the maximum and minimum numbers of links possible in each cluster.
...
Hence, nodes that belong to a lot of different communities will get the largest community centrality scores, whereas nodes that belong to overlapping or nested communities, or to few or no communities, will receive the lowest community centrality scores.\\
...
Cytoscape is an open source platform for complex-network analysis and visualization \citep{clineetal2007}. We can export our link communities into an edge attribute file that can be imported into Cytoscape:
